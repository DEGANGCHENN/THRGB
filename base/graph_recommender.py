...
class GraphRecommender(Recommender):
    ...
    def fast_evaluation_test(self, epoch):
        print('Evaluating the model...')
        rec_list = self.test()
        measure = ranking_evaluation(self.data.test_set, rec_list, [self.max_N])
        
        print('-' * 120)
        print('Test Ranking Performance ' + ' (Top-' + str(self.max_N) + ' Item Recommendation)')
    ...
    def fast_evaluation_valid(self, epoch, model, model_path):
        print('Evaluating the model...')
        rec_list = self.valid()
        ...
        print('-' * 120)
        if epoch - self.bestPerformance[0] >= 49:
            stop = True
        else:
            stop = False
        return stop
